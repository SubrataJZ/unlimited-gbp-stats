/**
 * Popup Script - Business switcher + stats overview
 */
  ...
  try {
    ...
    for (const biz of businesses) {
      ...
      item.innerHTML = `
        ...
        <div class="biz-name">${escapeHtml(biz.name || biz.id)}</div>
        <div class="biz-records">${records} records</div>
      `;
      item.addEventListener('click', () => {
        chrome.tabs.create({
          url: chrome.runtime.getURL(`dashboard.html?business=${biz.id}`)
        });
        window.close();
      });
